    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            LittleLemonTheme {
                ...
                val databaseMenuItems = database.menuItemDao().getAll().observeAsState().value
                ...
                val menuItems = if(orderMenuItems) databaseMenuItems?.sortedBy { it.title } else databaseMenuItems
                ...
                Column(
                    ...
                    OutlinedTextField(
                        ...
                        colors = TextFieldDefaults.textFieldColors(
                            ...
                            focusedLabelColor = Color(0xFF97740B),
                    ...
                    if (menuItems != null) {
                        MenuItemsList(items =if(searchPhrase.isEmpty()) menuItems else menuItems.filter { it.title.contains(searchPhrase, true) })
                    }
                }
            }
        }
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            if (database.menuItemDao().isEmpty()) {
                try {
                    val menuItem = fetchMenu() // Assuming this returns a List<MenuItemNetwork>
                    val menuItems = menuItem.menu
                    saveMenuToDatabase(menuItems)
                    Log.d("Fethcmenu", "fetching success")
                } catch (e: JsonConvertException) {
                    // Handle JSON parsing error gracefully
                    Log.e("MenuFetchError", "Error parsing JSON: ${e.localizedMessage}")
                    // Provide appropriate feedback to the user
                } catch (e: Exception) {
                    // Handle other potential errors
                    Log.e("MenuFetchError", "Unexpected error: ${e.localizedMessage}")
                }
            }
        }
    }
